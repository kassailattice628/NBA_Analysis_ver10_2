function [index_1, index_n] = GetIndex2(centers,ROI_Polar,dist_set,roi)
...
for n = 1:5
    ...
    %disp(rad2deg(phi));
    %-phi 回転後の stimn 応答中心
    [X0n_rot(n,1), Y0n_rot]= pol2cart(phi - phi, rho);
    ...
    %n応答中心に平行移動して極座標に戻す
    [ROI_Polar_rotn(:,1,n),ROI_Polar_rotn(:,2,n)] = cart2pol(Xn_rot-X0n_rot(n,1), Yn_rot-Y0n_rot);
end
%座標check
%{
figure;
...
ROI_Cart_rotn = ROI_Polar_rot;%ROI_Polar_rot を 平行移動して stimn を中心にした座標

for n = 1:5
    if X0n_rot(n,1) < dist_set
        dist = X0n_rot(n,1);
    else
        dist = dist_set;
    end
    [ROI_Cart_rot(:,1,n), ROI_Cart_rot(:,2,n)] = pol2cart(ROI_Polar_rot(:,1,n), ROI_Polar_rot(:,2,n));
    i1 = find(ROI_Polar_rot(:,2,1) < dist & ROI_Cart_rot(:,1,n) > 0);
    
    [ROI_Cart_rotn(:,1,n), ROI_Cart_rotn(:,2,n)] = pol2cart(ROI_Polar_rotn(:,1,n), ROI_Polar_rotn(:,2,n));
    i2 = find(ROI_Polar_rotn(:,2,n) < dist & ROI_Cart_rotn(:,1,n) < 0);
    
    
    
    %index_1{1,n} = intersect(roi,i1);
